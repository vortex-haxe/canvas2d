...
import canvas.servers.AudioServer.IMixerData;
import canvas.servers.AudioServer.IAudioSourceData;
import canvas.servers.AudioServer.IAudioBufferData;
import canvas.servers.AudioServer.MixerBackend;
...
class OpenALBackend extends MixerBackend {
	...
	override function init():Void {
		...
		if(device == null) {
			Logs.trace('Failed to open an OpenAL device.', ERROR);
			return;
		}
		final context:Context = ALC.createContext(device, null);
		if(!ALC.makeContextCurrent(context)) {
			Logs.trace('Failed to create OpenAL context.', ERROR);
			return;
		}

		final error:Int = AL.getError();
		if(error != AL.NO_ERROR) {
			Logs.trace('Failed to make OpenAL context current: ${error}', ERROR);
			return;
		}
